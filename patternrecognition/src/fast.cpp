...
void brute(int &N, vector<Point> &points, QGraphicsScene *scene, QApplication &a) {
...
void fast(vector<Point> &points, QGraphicsScene *scene, QApplication &a){
    ...
    while(points.size() > 3) {
        ...
        vector<Point> line; // Vector containing all points in the line.

        for(Point q: points){
            if(seqPoints == 0) {
                seqPoints++;
                line.push_back(q);
            } else if (p.slopeTo(q) == p.slopeTo(prevPoint)){
                seqPoints++;
                line.push_back(q);
                if(seqPoints >= 3) {
                    for(Point r: line){
                        render_line(scene, p, r);
                    }
                    a.processEvents(); // Draw the line
                    line.clear();
                }
            } else {
                seqPoints = 1;
